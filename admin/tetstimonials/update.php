...
$sql="select*from tetstimonals where id='$id'";
...
if(isset($_POST['submit'])){
    ...
    if($photo['name']){
        ...
        $ext="../../upload/tetstimonals/".$data['photo'];
        if(file_exists($ext)){
           
          }
         $sql="UPDATE tetstimonals SET name='$name',title='$title',discription='$discription',photo='$photoname' WHERE id=$id";            
    ...
    $sql="UPDATE tetstimonals SET name='$name',title='$title',discription='$discription' WHERE id=$id";           
     $database=mysqli_query($conn, $sql);
                    if($database){
                         header("Location:index.php");
                    }else{
